<template>
<div>
  <van-list
      v-model="loading"
      :finished="finished"
      :finished-text="load_msg"
      @load="onLoad"
  ><van-cell-group
      v-for="item in weixin_list"
      :key="item"
  >
    <van-cell :title="item.title" :label="item.brief" :size="large" :border="false" :url="item.url" title-class="weixin-title">
    <template #right-icon>
      <img  v-if="item.is_img" :src="item.img_url" class="weixin-img"/>
    </template>

      <template #label>
        <div class="img-3-div" v-if="item.is_img_3">
          <img v-for="img_item in item.img_url" v-bind:key="img_item" :src="img_item" class="img-3"/>
        </div>
        <div v-if="!item.is_img_3">{{item.brief}}</div>
      </template>
    </van-cell>
    <div class="weixin-info"><img class="left-icon" src="https://tangwei.cc/imgs/weixin.png" />{{item.biz_name}} {{item.issue_date_str}}</div>
    <div class="weixin-info">阅读量：{{format_num(item.read_num)}}</div>
  </van-cell-group>

  </van-list>
</div>
</template>

<script>
import { List } from 'vant';
import { Cell, CellGroup } from 'vant';
import weixinApi from "@/api/weixin.js";
export default {
  name: "weixin",
  data() {
    return {
      weixin_list: [],
      loading: false,
      finished: false,
      query_data: {
        limit: 10,
        offset: 0,
        time_limit: 0,
        order_by: 'read_num',
        order_sort: 'desc',
        mode: 1
      },
      load_msg: this.$root.current_limit_days.toString() + "天之内没有更多了..."
    }
  },
  methods: {
    onLoad() {
      // 在全局变量中标注当前路径
      this.$root.current_path = this.$route.path;
      console.log("current_path:", this.$root.current_path)
      this.load_data();
    },
    load_data() {
      // 利用全局变量得到时间戳
      this.query_data.time_limit = this.GLOBAL.get_timelimt_by_day(this.$root.current_limit_days);
      // 将搜索天数发送回后天，便于智能排序
      this.query_data.current_limit_days = this.$root.current_limit_days;
      // 异步更新数据
      weixinApi
          .get_weixin(this.query_data)
          .then(res => {
            console.log('res:', res);
            const data = res.data;
            for (let i = 0; i < data.length; i++) {
              this.weixin_list.push(data[i]);
            }
            //将offset +1
            this.query_data.offset = this.query_data.offset + 1;
            this.loading = false;
            // // 加载状态结束
            // this.loading = false;
            // // 数据全部加载完成
            if (this.weixin_list.length >= 100 || res.data.length == 0) {
              this.finished = true;
            }
          })
    },
    format_num(num) {
      if(num > 100000){
        return "10万+";
      }else if(num > 10000){
        const w = num / 10000;
        const w1 = w.toFixed(1);
        const w1_str = w1.toString() + "万";
        return w1_str;
      }else {
        return num.toString();
      }
    }
  },
  watch: {
    '$root.current_limit_days': {
      handler() {
        this.weixin_list = [];
        this.loading = false;
        this.finished = false;
        this.query_data = {
          limit: 10,
          offset: 0,
          time_limit: 0,
          order_by: 'read_num',
          order_sort: 'desc',
          mode: 1
        };
        this.load_msg = this.$root.current_limit_days.toString() + "天之内没有更多了...";
        this.load_data();
      }
    }
  },
  components: {
    vanList: List,
    vanCell: Cell,
    vanCellGroup: CellGroup
  }

}
</script>

<style scoped>
.weixin-img {
  height: 100px;
  width: 100px;
  margin: 5px;
}

.img-3 {
  height: 100px;
  width: 100px;
  float: left;
  margin: 2px;
}

.weixin-info {
  margin-left: 20px;
  font-size: 14px;
  color: darkgrey;
}

.weixin-title{
  margin-top: 5px;
  font-size: 20px;
}

.left-icon {
  height: 12px;
  width: 12px;
  margin-top: 10px;
  margin-right: 2px;
}

</style>