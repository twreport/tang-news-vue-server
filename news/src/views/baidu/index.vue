<template>
  <div>
    ...
    >
      <van-cell-group
          v-for="item in baidu_list"
          :key="item"
      >
        <van-cell :title="item.word" :size="large" :url="item.url" title-class="baidu-title">
          <template #right-icon>
        ...
        <div class="baidu-info">热度：{{ item.hotScore }}</div>
      </van-cell-group>
...
import baiduApi from "@/api/baidu.js";
...
export default {
  ...
  methods: {
    ...
    load_data() {
      ...
      // 异步更新数据
      baiduApi
          .get_baidu(this.query_data)
          .then(res => {
            console.log('res:', res);
            const data = res.data;
            for (let i = 0; i < data.length; i++) {
              this.baidu_list.push(data[i]);
            }
            //将offset +1
            ...
            // this.loading = false;
            // // 数据全部加载完成
            if (this.baidu_list.length >= 100 || res.data.length == 0) {
              this.finished = true;
            }
          })
    }
